...
function Hero() {
  ...
  const submitPrompt = event => { event.preventDefault(); if (!prompt.trim()) return; setSent(true); setPrompt('') }
  return <ClickSpark sparkColor="#eeeaff" sparkSize={7} sparkRadius={28} sparkCount={10} duration={520} extraScale={1.1}><section className="hero" aria-label="NightSheep AI"><div className="hero-veil" /><div className="hero-visual" aria-hidden="true"><div className="hero-wordmark"><ParticleText text="NightSheep AI" particleSize={2.1} density={4} color="#f7f4ff" highlightColor="#ffffff" scatter={150} gatherDuration={1500} stagger={280} pointerRepel={34} repelRadius={150} idleDrift={.8} trigger="hover" fontSize="clamp(7rem, 15.8vw, 19rem)" fontWeight={600} fontFamily="MPlus, MiSans, sans-serif" glow /></div><div className="hero-scanlines" /><div className="hero-haze" /></div><CursorDot /><div className="hero-copy"><div className="hero-subtitle"><BlurText text="Seeking the optimal conversion from energy to intelligence" delay={24} animateBy="words" direction="bottom" stepDuration={.22} /></div><form className="prompt-box" onSubmit={submitPrompt}><input value={prompt} onChange={event => { setPrompt(event.target.value); setSent(false) }} placeholder="Throw me a hard one. I’m ready." aria-label="Tell NightSheep your challenge" /><button type="submit" aria-label="发送">{sent ? '✓' : '↑'}</button></form></div></section></ClickSpark>
}
